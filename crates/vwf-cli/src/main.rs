...
fn parse_key_val(s: &str) -> std::result::Result<(String,String), String> {
    ...
    if parts.len() != 2 { return Err("expected key=value".into()); }
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Run { workflow, workdir, vars, dry_run, allow, mock_llm_canned } => {
            let text = std::fs::read_to_string(&workflow).with_context(|| format!("read {}", workflow.display()))?;
            let cfg: WorkflowConfig = serde_yaml::from_str(&text).context("parse workflow yaml")?;
            ...
            if dry_run {
                ...
                for d in rt.planned_dirs { println!("{d}"); }
                println!("--- planned writes ---");
                for (p, _) in rt.planned_writes { println!("{p}"); }
                Ok(())
            } else {
