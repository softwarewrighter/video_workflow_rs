...
pub fn execute_step(rt: &mut dyn Runtime, vars: &BTreeMap<String, String>, step: &StepConfig) -> Result<()> {
    match step.kind {
        ...
        StepKind::WriteFile => {
            #[derive(Deserialize)]
            struct Payload { path: String, content: String }
            let p: Payload = serde_json::from_value(step.payload.clone())
        ...
        StepKind::SplitSections => {
            #[derive(Deserialize)]
            struct Output { heading: String, path: String }
            #[derive(Deserialize)]
            struct Payload { input_path: String, outputs: Vec<Output> }
            let p: Payload = serde_json::from_value(step.payload.clone())
            ...
            for out in p.outputs {
                ...
                let section = extract_section(&text, &heading)
                    .with_context(|| format!("step `{}` cannot extract heading `{heading}`", step.id))?;
        ...
        StepKind::RunCommand => {
            #[derive(Deserialize)]
            struct Payload { program: String, #[serde(default)] args: Vec<String>, #[serde(default)] cwd: Option<String>, #[serde(default)] capture_path: Option<String> }
            let p: Payload = serde_json::from_value(step.payload.clone())
                .with_context(|| format!("step `{}` payload decode run_command", step.id))?;
            let program = render_template(&p.program, vars)?;
            let args: Vec<String> = p.args.iter().map(|a| render_template(a, vars)).collect::<Result<_>>()?;
            let cwd = match &p.cwd { Some(c) => Some(render_template(c, vars)?), None => None };
        ...
        StepKind::LlmGenerate => {
            ...
            struct Payload {
                ...
                output_path: String,
                provider: String,
                #[serde(default)] mock_response: Option<String>,
            }
            let p: Payload = serde_json::from_value(step.payload.clone())
            ...
            let mut req = LlmRequest { system, user, provider };
...
fn extract_section(text: &str, heading: &str) -> Result<&str> {
