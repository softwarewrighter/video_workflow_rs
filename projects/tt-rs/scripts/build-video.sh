...
# Volume normalization is done AFTER clip creation using normalize-volume.sh
# This follows the proven pattern from explainer/shorts projects
# Target: -25 dB (matches epilog reference)
# Music clips: -32 dB (7 dB quieter than speech)
...
create_section_clip() {
    ...
    if [ -f "$audio" ]; then
        ...
        # Volume normalization done in post-processing step
...
# ============================================
# Step 6: Normalize volume levels (post-process)
# ============================================
echo "Step 6: Normalizing volume levels..."

# Narration clips: normalize to -25 dB (match epilog)
for clip in 01-hook.mp4 02-intro-demo.mp4 03-cta.mp4; do
    "$SCRIPT_DIR/normalize-volume.sh" "$CLIPS_DIR/$clip" -25
done

# Music clips: normalize to -32 dB (7 dB quieter than speech)
for clip in 00-title.mp4 99b-epilog-ext.mp4; do
    "$SCRIPT_DIR/normalize-volume.sh" "$CLIPS_DIR/$clip" -32
done

# Epilog already at reference level, just ensure format
"$SCRIPT_DIR/normalize-volume.sh" "$CLIPS_DIR/99a-epilog.mp4" -25

# Demo placeholder is silent, just ensure format
"$SCRIPT_DIR/normalize-volume.sh" "$CLIPS_DIR/03-demo.mp4" -25
